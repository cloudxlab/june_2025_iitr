...
## Tower of hanoi using recursion. First solve it for 1 disc, then solve it for 2 discs and then solve it for 3 disc. Afterwards, generalize it for n by using the solution for n-1 discs. Give the names A for source, B for Auxulary and C for Target. The top most disc is labeled smallest. It should print on the screen: Moving 1 from A to C.
